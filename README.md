...
Electronics RFQ Agent isn't published to PyPI yet. The release pipeline is wired up and ready to go; see the FAQ if you want the details. Until then, install from source:
...
|---|---|---|
| Claude (built-in) | `pip install -e .` (source, see [Install](#install)) | [01-basic-quote](examples/01-basic-quote/) |
| LangGraph | `pip install -e '.[langgraph]'` | [04-langgraph-agent](examples/04-langgraph-agent/) |
| OpenAI Agents SDK | `pip install -e '.[agents]'` | [05-openai-agents](examples/05-openai-agents/) |
| CrewAI | `pip install -e '.[crewai]'` | n/a |
...
It's Beta (v0.1.x). The core library, Epicor, Oracle, and Dynamics connectors are covered by CI (lint, mypy strict, pytest with an 80% coverage gate, Trivy scanning) across Python 3.10-3.12. SAP support is explicitly marked Beta and requires a manual install step (the SAP NetWeaver RFC Library isn't on PyPI). It hasn't had a tagged release yet; see [Install](#install).
...
The publish pipeline (build, Sigstore signing, SBOM, trusted-publisher upload) is already wired up in `.github/workflows/release.yml` and triggers on a `v*` tag push. It just hasn't been run yet. Until then, install from source per [Install](#install).
